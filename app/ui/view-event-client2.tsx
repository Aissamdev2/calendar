...
export default function ViewEventClient({ id }: { id: string | undefined }) {
  ...
  if (!subjects) return null
    return (
      <form action={dispatch} className="starting:scale-[0] scale-[1] transition-[transform] duration-300 w-full h-full max-h-screen fixed top-0 left-0 z-[1000] overflow-x-hidden overflow-y-auto">
      <input type="hidden" name="id" value={event.id} />
      <div className="w-fit max-h-[80%] md:h-auto relative top-1/2 left-1/2 translate-x-[-50%] translate-y-[-50%] bg-white p-6">
